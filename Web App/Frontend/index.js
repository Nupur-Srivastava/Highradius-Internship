...
function delete_details() {
    const checked = $('tr.c1 > td > input:checked');
    if (!checked.length) return;

    checked.map(async function() {
        const check = $(this);
        const tr = check.first().closest('tr');
        const invoice =  tr.children().eq(4).html();
        console.log(check,tr,invoice)
        await fetch(url + "/delete", {
            method: "POST",
            body: new URLSearchParams({ invoice }),
        });
    });
